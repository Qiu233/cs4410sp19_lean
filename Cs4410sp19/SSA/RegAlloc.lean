...
def annotate_lineno [Hashable γ] [BEq γ] : CFG' Unit γ δ α → CFG' Nat γ δ α := fun cfg => Id.run do
  ...
  for b in cfg.blocks do
    ...
    for inst in b.insts do
      ...
      i := i + 1
    bs := bs.push (BasicBlock.mk b.id b.params xs)
...
def RegAlloc.build_intervals (cfg : CFG' Nat String VarName Operand) : Std.HashMap VarName Lifetime := Id.run do
  ...
  for b in blocks.reverse do
    ...
    match last with
    | .jmp _ _ args =>
      live := live.insertMany (args.filterMap isVar?)
    | .br _ _ _ pargs _ nargs =>
      live := live.insertMany (pargs.filterMap isVar?)
      live := live.insertMany (nargs.filterMap isVar?)
    | _ => pure ()
    for opd in live do
    ...
    for op in b.insts.reverse do
      for opd in op.output_operands do
        ...
        live := live.erase opd
      for opd in op.input_operands.filterMap isVar? do
        intervals := intervals.alter opd fun
          | none => some Lifetime.empty
          | some x => some x
        let used := b.insts.filterMap fun x => if x.input_operands.contains (.var opd) then some x.tag else none
  ...
  for b in cfg.blocks do
    ...
    for inst in b.insts do
      let r : Inst Nat String Dst Src :=
        ...
        | .br       tag cond bp pargs bn nargs    => .br tag (get_src cond) bp (pargs.map get_src) bn (nargs.map get_src)
        | .jmp      tag target xs                 => .jmp tag target (xs.map get_src)
      ...
      insts' := insts'.push r
    blocks' := blocks'.push { id := b.id, params := [], insts := insts' : BasicBlock Nat String Dst Src }
